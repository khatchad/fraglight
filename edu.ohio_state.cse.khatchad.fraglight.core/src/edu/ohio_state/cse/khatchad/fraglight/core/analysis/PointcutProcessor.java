...
public abstract class PointcutProcessor {
		...
		for (final Iterator it = suggestedArcs.iterator(); it.hasNext();) {
			...
			if (!patternToResultMap.containsKey(pattern))
				patternToResultMap.put(pattern,
						new LinkedHashSet<GraphElement<IElement>>());
	...
	protected ConcernGraph createConcernGraph(final Collection<IProject> projectsToAnalyze, final IProgressMonitor monitor,
			TimeCollector timeCollector) throws Exception {
		final ConcernGraph graph = new ConcernGraph(projectsToAnalyze, this.maximumAnalysisDepth, monitor, timeCollector);
		return graph;
	}

	protected abstract void analyzeAdviceCollection(
			final Collection<? extends AdviceElement> adviceCol,
			final ConcernGraph graph, final IProgressMonitor monitor, TimeCollector timeCollector) throws ConversionException,
	 ...
	 * @return
	 */
	private Element getXML(final Set<GraphElement<IElement>> set,
			String elementName) {
	...
	protected void executeQueries(
		...
		executeNodeQuery(new SubProgressMonitor(monitor, 1,
	...
	/**
	 * @param maximumAnalysisDepth the maximumAnalysisDepth to set
	 */

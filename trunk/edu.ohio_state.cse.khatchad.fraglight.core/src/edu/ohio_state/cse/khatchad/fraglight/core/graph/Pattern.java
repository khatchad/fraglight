...
public class Pattern<E extends IntentionArc<IElement>> extends Path<E> {
	...
	private double getConcreteness() {
		...
		final Collection<IntentionNode<IElement>> wildNodes = this 
				.getWildcardNodes();
		return (double) (allNodes.size() - wildNodes.size()) / allNodes.size();
	}

	private static double calculatePrecision(
			final Set<GraphElement<IElement>> searchedFor,
	...
	private static double performSimularityCalculation(final double precision,
			final double coverage, final double concreteness) {
	...
	/**
	 * Calculates the similarity to the associated advice based on the given results from matching this pattern.
	 * @param patternResults Elements matching the pattern.
	 * @param patternEnabledResults Elements matching the pattern that are enabled by the associate advice.
	 * @param graph The concern graph used to produce the results.
	 * @return The pattern simularity; also sets in the pattern.
	 */
	public double calculateSimularityToAdviceBasedOnResults(
			final Set<GraphElement<IElement>> patternResults,
			final Set<GraphElement<IElement>> patternEnabledResults,
			ConcernGraph graph)  {

		double precision = calculatePrecision(patternEnabledResults,
				patternResults);

		double coverage = patternEnabledResults.size()
				/ graph.getEnabledElements().size();

		double concreteness = this.getConcreteness();

		double simularity = performSimularityCalculation(precision, coverage,
				concreteness);
